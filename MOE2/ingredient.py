...
class Ingredient(OSDObject):
    ...
    def __init__(self, row, column, id, palette, mutable=False):
        super().__init__(row.scene, id)
        if palette is None:
            self._palette = row.window.default_palette
        else:
            self._palette = row.window.find_palette(palette)
